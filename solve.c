...
int check_solvable(char ** game) {
    ...
    for (int y = 0; y < 9 && !stop; y++ ) {
        for (int x = 0; x < 9 && !stop; x++) {
            if (game[y][x] == 0) {
                empty_x = x;
                empty_y = y;
                stop = TRUE;
    ...
    // If no field is empty that means the grid is solvable
    if (empty_x == -1 && empty_y == -1) return TRUE;

    // Get the box from the empty flield
    show_game(game);
    int empty_box = get_box_num(empty_x, empty_y);
    ...
    for (val = 1; val <= 9; val++) {
        ...
        if (!in_row && !in_col && !in_box) {
            game[empty_x][empty_x] = val;
            int possible = check_solvable(game);
            ...
            if (possible == TRUE) {
                ...
                // If the game is not possible the undo the change and go to 
                // the next value
                game[empty_y][empty_y] = 0;
            }
